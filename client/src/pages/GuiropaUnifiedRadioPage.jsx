...
export default function GuiropaUnifiedRadioPage({ initialSource = "guiropa" }) {
  ...
  useEffect(() => {
    if (streamRef.current) streamRef.current.volume = streamVolume;
  }, [streamVolume]);
  ...
  function togglePlayback() {
    ...
    if (streamPlaying) pauseStream();
    else void playStream();
  }

  function selectSource(nextId) {
    if (nextId === sourceId) return;
    if (isCatalog) radio.pause();
    else pauseStream();
    setSourceId(nextId);
  }
  ...
  function previousSource() {
    const prevIndex = (sourceIndex - 1 + SOURCES.length) % SOURCES.length;
    selectSource(SOURCES[prevIndex].id);
  }
  ...
  function changeVolume(value) {
    ...
    if (isCatalog) radio.setVolume(nextValue);
    else setStreamVolume(nextValue);
  }
      ...
      <div className="gurx__shell">
        ...
        <section className="gurx__receiver" aria-label="GUIROPA unified radio receiver">
          <div className="gurx__main">
            ...
            <div className="gurx__display">
              <div className="gurx__status"><span className="gurx__dot" />{statusText}</div>
              <div>
                <div className="gurx__group">{source.group}</div>
                <h2 className="gurx__title">{displayTitle}</h2>
                <div className="gurx__artist">{displayArtist}</div>
              </div>
            </div>
            <div className="gurx__tuning">
              <div className="gurx__tuning-head">GUIROPA SIGNAL SELECTOR</div>
              <div className="gurx__dial">
                <div className="gurx__dial-labels">{SOURCES.map((item) => <span key={item.id}>{item.short}</span>)}</div>
                <div className="gurx__scale"><span className="gurx__needle" style={{ left: `${(sourceIndex / (SOURCES.length - 1)) * 100}%` }} /></div>
              </div>
              <div className="gurx__banks">
                {SOURCES.map((item) => (
                  <button key={item.id} className={`gurx__bank ${item.id === sourceId ? "is-active" : ""}`} type="button" onClick={() => selectSource(item.id)}>{item.label}</button>
                ))}
              </div>
            </div>
          </div>

          <div className="gurx__controls">
            <div className="gurx__transport">
              <button className="gurx__round" type="button" onClick={previousSource} aria-label="Sinal anterior">◀</button>
              <button className="gurx__play" type="button" onClick={togglePlayback} disabled={loading} aria-label={playing ? "Pausar" : "Tocar"}>{loading ? "…" : playing ? "Ⅱ" : "▶"}</button>
              <button className="gurx__round" type="button" onClick={next} aria-label={isCatalog ? "Próxima faixa" : "Próximo sinal"}>▶▶</button>
            </div>
            <div className="gurx__copy"><strong>GET UP. TURN IT UP. GUIROPA.</strong><span>{isCatalog ? "Arquivo ao vivo GUIROPA. Próxima faixa sorteada automaticamente." : `${source.group} · ${source.subtitle}`}</span></div>
            <div className="gurx__volume"><button type="button" onClick={toggleMute} aria-label="Som">{muted ? "×" : "◖"}</button><input type="range" min="0" max="1" step="0.01" value={volume} onChange={(event) => changeVolume(event.target.value)} /></div>
          </div>
        ...
        <div className="gurx__legend">
          {SOURCES.map((item) => <button key={item.id} type="button" className={item.id === sourceId ? "is-active" : ""} onClick={() => selectSource(item.id)}><strong>{item.label}</strong><span>{item.group}</span></button>)}
        </div>
